...
def check_pr_mergeable(owner: str, repo: str, pr_number: int, token: str) -> Optional[bool]:
...
def analyze_pr(pr: Dict, owner: str, repo: str, token: str) -> Dict:
    ...
    analysis = {
        ...
        'mergeable': mergeable,
        'is_daily_update': 'Daily Update' in pr['title'] or 'daily-contribution' in pr['title'],
...
def display_pr_analysis(analyses: List[Dict]) -> None:
    ...
    if ready_to_merge:
        ...
        for pr in ready_to_merge:
            ...
            print(f"    Files: {', '.join(pr['files'][:3])}{'...' if len(pr['files']) > 3 else ''}")
    ...
    if has_conflicts:
        ...
        for pr in has_conflicts:
            print(f"  #{pr['number']}: {pr['title']}")
            print(f"    Action needed: Resolve conflicts manually")
            print()
    ...
    if review_manually:
        ...
        for pr in review_manually:
            print(f"  #{pr['number']}: {pr['title']}")
            print(f"    Note: Not a daily update PR")
            print()
...
def show_achievement_progress(merged_count: int) -> None:
    ...
    for achievement in achievements:
        ...
        if 'progress' in achievement:
            progress_bar = "█" * achievement['progress'] + "░" * (achievement['total'] - achievement['progress'])
            print(f"   Progress: [{progress_bar}] {achievement['progress']}/{achievement['total']}")
